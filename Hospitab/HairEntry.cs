...
namespace Hospitab
{
    ...
    public class HairEntry : AppCompatActivity
    {
        Spinner cmbfam;
        Spinner cmbdan;
        Spinner cmbtel;
        ...
        ListView LView;
        MultiAutoCompleteTextView txthist;
        List<Medlist> tb = new List<Medlist>();

        protected override void OnCreate(Bundle savedInstanceState)
        {
            base.OnCreate(savedInstanceState);
            SetContentView(Resource.Layout.hair_entry);
            Android.Support.V7.Widget.Toolbar toolbar = FindViewById<Android.Support.V7.Widget.Toolbar>(Resource.Id.dtoolbar24);
            SetSupportActionBar(toolbar);
            Name = (TextView)FindViewById(Resource.Id.lbltname);
            Visitno = (TextView)FindViewById(Resource.Id.lbltvisit);
            ...
            Name.Text = pname;
            Age = (TextView)FindViewById(Resource.Id.lbltage);
            string page = Intent.GetStringExtra("Age") ?? string.Empty;
            Age.Text = page;
            cmbfam = (Spinner)FindViewById(Resource.Id.cmbfam);
            ...
            cmbfam.Adapter = adapter;
            cmbdan = FindViewById<Spinner>(Resource.Id.cmbdan);
            ...
            cmbdan.Adapter = adapter1;
            cmbtel = (Spinner)FindViewById(Resource.Id.cmbtel);
            ...
            cmbtel.Adapter = adapter2;
            cmbtime = (Spinner)FindViewById(Resource.Id.cmbttime);
            ...
            LoadMed();
            LoadAdv();
            txtcfeat = FindViewById<MultiAutoCompleteTextView>(Resource.Id.txttcfeat);
            txtdiag = FindViewById<MultiAutoCompleteTextView>(Resource.Id.txttdiag);
            txttreat = FindViewById<MultiAutoCompleteTextView>(Resource.Id.txtttreat);
            txthist = FindViewById<MultiAutoCompleteTextView>(Resource.Id.txthist);
            txtmed = FindViewById<AutoCompleteTextView>(Resource.Id.txttmed);
            txtdose = FindViewById<EditText>(Resource.Id.txttdose);
            txtadvice = FindViewById<AutoCompleteTextView>(Resource.Id.txttadvice);
            NRemarks = FindViewById<EditText>(Resource.Id.txttremarks);
            BtnSave = FindViewById<Button>(Resource.Id.btntSave);
            btnImg = FindViewById<Button>(Resource.Id.btntimg);
            BtniAdd = FindViewById<Button>(Resource.Id.btntAdd);
            imgpresc = FindViewById<ImageView>(Resource.Id.imgtpower);
            LView = FindViewById<ListView>(Resource.Id.presct_listView);
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            string fam = cmbfam.GetItemAtPosition(cmbfam.SelectedItemPosition).ToString();
            string dan = cmbdan.GetItemAtPosition(cmbdan.SelectedItemPosition).ToString();
            string tel = cmbtel.GetItemAtPosition(cmbtel.SelectedItemPosition).ToString();
            string ncfeat = txtcfeat.Text.Trim();
            string ndiag = txtdiag.Text.Trim();
            string ntreat = txttreat.Text.Trim();
            string hist = txthist.Text.Trim();
            ...
            t3.HairCompleted += T3_HairCompleted;
            t3.HairAsync(Globals.docid, Globals.gyear, Globals.gmonth, Globals.gday, Globals.gageflag, Globals.regno, fam, dan, tel, hist.TrimEnd(','), ncfeat.TrimEnd(','), ndiag.TrimEnd(','), ntreat.TrimEnd(','), tremarks, bal, "0", gpresc, "Dermatology");
        ...
        private void T3_HairCompleted(object sender, titaniumdoc.HairCompletedEventArgs e)
        {
